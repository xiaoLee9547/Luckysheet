...
export function exportSheetExcel(luckysheet, name) { // 参数为luckysheet.getluckysheetfile()获取的对象
    ...
    const tableArr = luckysheet.getAllSheets()
...
const getBorderInfo = function (luckysheetfile) {
    ...
    // 设置需要计算边框的区域
    const dataset_row_st = 0
    const dataset_row_ed = data.length
    const dataset_col_st = 0
    const dataset_col_ed = data[0].length
            ...
            if (rangeType === 'range') {
                ...
                for (let j = 0; j < borderRange.length; j++) {
                    ...
                    if (borderType === 'border-left') {
                        for (let bd_r = bd_r1; bd_r <= bd_r2; bd_r++) {
                            if (cfg.rowhidden && cfg.rowhidden[bd_r]) {
                                continue
                            }
                            ...
                            if (bd_c_left >= 0 && borderInfoCompute[`${bd_r}_${bd_c_left}`]) {
                                if (data[bd_r]  && getObjType(data[bd_r][bd_c_left]) === 'object' && data[bd_r][bd_c_left].mc ) {
                        ...
                        for (let bd_r = bd_r1; bd_r <= bd_r2; bd_r++) {
                            ...
                            if (bd_c_right < data[0].length && borderInfoCompute[`${bd_r}_${bd_c_right}`]) {
                                if (data[bd_r]  && getObjType(data[bd_r][bd_c_right]) === 'object' && data[bd_r][bd_c_right].mc ) {
                        ...
                        if (cfg.rowhidden  && cfg.rowhidden[bd_r1] ) {
                            continue
                        }
                        ...
                        for (let bd_c = bd_c1; bd_c <= bd_c2; bd_c++) {
                            ...
                            if (bd_r_top >= 0 && borderInfoCompute[`${bd_r_top}_${bd_c}`]) {
                                if (data[bd_r_top]  && getObjType(data[bd_r_top][bd_c]) === 'object' && data[bd_r_top][bd_c].mc ) {
                        ...
                        if (cfg.rowhidden  && cfg.rowhidden[bd_r2] ) {
                            continue
                        }
                        ...
                        for (let bd_c = bd_c1; bd_c <= bd_c2; bd_c++) {
                            ...
                            if (bd_r_bottom < data.length && borderInfoCompute[`${bd_r_bottom}_${bd_c}`]) {
                                if (data[bd_r_bottom]  && getObjType(data[bd_r_bottom][bd_c]) === 'object' && data[bd_r_bottom][bd_c].mc ) {
                        ...
                        for (let bd_r = bd_r1; bd_r <= bd_r2; bd_r++) {
                            ...
                            for (let bd_c = bd_c1; bd_c <= bd_c2; bd_c++) {
                                if (data[bd_r]  && getObjType(data[bd_r][bd_c]) === 'object' && data[bd_r][bd_c].mc ) {
                ...
                if (value.l  || value.r  || value.t  || value.b ) {
                    ...
                    if (data[bd_r]  && getObjType(data[bd_r][bd_c]) === 'object' && data[bd_r][bd_c].mc ) {
                        ...
                        if (value.r  && bd_c === mc.c + mc.cs - 1) { // 右边框
                        ...
                        if (value.b  && bd_r === mc.r + mc.rs - 1) { // 下边框
...
const fillConvert = function (bg) {
    ...
    bg = bg.includes('rgb') ? rgb2hex(bg) : bg
    return {
        type: 'pattern',
        pattern: 'solid',
